...
const myProjects = [
    {
        ...
        color: "text-pink-500",
        bg: "bg-pink-500/10",
        isFullPage: false, // Standard separated HTML/CSS/JS
        html: `
    ...
    },
    {
        ...
        description: "Browser-based document scanner with smart B&W filter, PDF export, and edge detection simulation.",
        icon: "fa-file-pdf",
        color: "text-gray-200",
        bg: "bg-gray-800",
        isFullPage: true, // Indicates this contains a full HTML document structure
        html: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"><title>DocScanner Pro - Smart Scan</title><script src="https://cdn.tailwindcss.com"></script><script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script><script src="https://unpkg.com/lucide@latest"></script><style>::-webkit-scrollbar{width:6px}::-webkit-scrollbar-track{background:#111827}::-webkit-scrollbar-thumb{background:#374151;border-radius:3px}::-webkit-scrollbar-thumb:hover{background:#4b5563}input[type=range]{-webkit-appearance:none;width:100%;background:0 0}input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;height:16px;width:16px;border-radius:50%;background:#3b82f6;cursor:pointer;margin-top:-6px;box-shadow:0 0 0 2px #1f2937}input[type=range]::-webkit-slider-runnable-track{width:100%;height:4px;cursor:pointer;background:#4b5563;border-radius:2px}input[type=range]:focus{outline:0}.drop-zone{background-image:url("data:image/svg+xml,%3csvg width='100%25' height='100%25' xmlns='http://www.w3.org/2000/svg'%3e%3crect width='100%25' height='100%25' fill='none' rx='24' ry='24' stroke='%234B5563FF' stroke-width='2' stroke-dasharray='12%2c 12' stroke-dashoffset='0' stroke-linecap='square'/%3e%3c/svg%3e");transition:all .3s ease}.drop-zone:hover,.drop-zone.dragover{background-image:url("data:image/svg+xml,%3csvg width='100%25' height='100%25' xmlns='http://www.w3.org/2000/svg'%3e%3crect width='100%25' height='100%25' fill='none' rx='24' ry='24' stroke='%233B82F6FF' stroke-width='3' stroke-dasharray='12%2c 12' stroke-dashoffset='0' stroke-linecap='square'/%3e%3c/svg%3e");background-color:rgba(59,130,246,.05);transform:scale(1.01)}.spinner{border:3px solid rgba(255,255,255,.1);width:24px;height:24px;border-radius:50%;border-left-color:#3b82f6;animation:spin 1s linear infinite}@keyframes spin{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}.pb-safe{padding-bottom:env(safe-area-inset-bottom)}</style></head><body class="bg-gray-900 text-gray-100 h-[100dvh] flex flex-col font-sans overflow-hidden"><header class="flex-none bg-gray-800 border-b border-gray-700 px-4 py-3 z-20 shadow-lg"><div class="max-w-7xl mx-auto flex items-center justify-between"><div class="flex items-center gap-3"><div class="bg-blue-600/20 p-2 rounded-lg"><i data-lucide="scan-text" class="text-blue-400 w-5 h-5"></i></div><div><h1 class="text-lg font-bold tracking-tight text-white leading-tight">DocScanner</h1><p class="text-[10px] text-gray-400 font-medium uppercase tracking-wider">AI Enhanced</p></div></div><div class="flex items-center gap-3"><button id="reset-btn" class="hidden text-xs font-medium text-gray-400 hover:text-white transition px-3 py-1.5 rounded-md hover:bg-gray-700">Clear All</button> <span id="badge-count" class="hidden bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-lg shadow-blue-900/50">0</span></div></div></header><main class="flex-1 relative overflow-hidden flex flex-col w-full max-w-7xl mx-auto"><div id="upload-view" class="absolute inset-0 z-10 flex flex-col items-center justify-center p-6 bg-gray-900 transition-transform duration-300"><div id="drop-zone" class="drop-zone w-full max-w-xl aspect-[4/3] md:aspect-[16/9] rounded-3xl flex flex-col items-center justify-center cursor-pointer bg-gray-800/30"><input type="file" id="file-input" accept="image/*" multiple class="hidden"><div class="w-16 h-16 bg-gray-800 rounded-2xl flex items-center justify-center mb-6 shadow-xl border border-gray-700 group-hover:border-blue-500/50 transition-colors"><i data-lucide="upload" class="w-8 h-8 text-blue-400"></i></div><h2 class="text-2xl font-bold mb-2 text-white text-center">Scan Documents</h2><p class="text-gray-400 text-center mb-8 max-w-xs text-sm leading-relaxed">Drop images here or tap to select from gallery. <br><span class="text-gray-600">Supports JPG, PNG, WEBP</span></p><button class="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold shadow-lg shadow-blue-900/20 active:scale-95 transition-all flex items-center gap-2 text-sm"><i data-lucide="image-plus" class="w-4 h-4"></i> Select Images</button></div></div><div id="editor-view" class="absolute inset-0 z-0 flex flex-col bg-gray-900 translate-x-full transition-transform duration-300"><div id="pages-grid" class="flex-1 overflow-y-auto p-4 pb-32 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-min"></div><div class="absolute bottom-24 right-4 md:bottom-28 md:right-8 z-30"><button id="fab-add" class="bg-blue-600 hover:bg-blue-500 text-white w-14 h-14 rounded-full shadow-xl shadow-black/50 flex items-center justify-center transition-transform active:scale-90 border-2 border-white/10"><i data-lucide="plus" class="w-8 h-8"></i></button></div><div class="flex-none bg-gray-800 border-t border-gray-700 p-4 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-40"><div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center gap-4"><div class="text-xs text-gray-500 hidden sm:block"><i data-lucide="info" class="w-3 h-3 inline mr-1"></i>Adjust sliders for best B&W results.</div><button id="download-btn" class="w-full sm:flex-1 bg-white text-gray-900 hover:bg-gray-100 py-3.5 px-6 rounded-xl font-bold shadow-lg active:scale-[0.98] transition flex items-center justify-center gap-2"><i data-lucide="file-down" class="w-5 h-5 text-blue-600"></i> Download PDF</button></div></div></div></main><div id="loader" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex flex-col items-center justify-center transition-opacity opacity-0"><div class="bg-gray-800 p-8 rounded-2xl flex flex-col items-center border border-gray-700 shadow-2xl"><div class="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-500 mb-4"></div><p id="loader-text" class="text-white font-medium tracking-wide">Processing...</p></div></div><canvas id="work-canvas" class="hidden"></canvas><script>let pages=[],isProcessing=!1;const UI={uploadView:document.getElementById("upload-view"),editorView:document.getElementById("editor-view"),dropZone:document.getElementById("drop-zone"),fileInput:document.getElementById("file-input"),pagesGrid:document.getElementById("pages-grid"),downloadBtn:document.getElementById("download-btn"),fabAdd:document.getElementById("fab-add"),resetBtn:document.getElementById("reset-btn"),badgeCount:document.getElementById("badge-count"),loader:document.getElementById("loader"),loaderText:document.getElementById("loader-text"),canvas:document.getElementById("work-canvas")},ctx=UI.canvas.getContext("2d",{willReadFrequently:!0});lucide.createIcons(),UI.dropZone.addEventListener("click",(()=>UI.fileInput.click())),UI.fabAdd.addEventListener("click",(()=>UI.fileInput.click())),UI.resetBtn.addEventListener("click",confirmReset),UI.downloadBtn.addEventListener("click",generatePDF);const prevent=e=>{e.preventDefault(),e.stopPropagation()};["dragenter","dragover","dragleave","drop"].forEach((e=>{UI.dropZone.addEventListener(e,prevent)})),["dragenter","dragover"].forEach((e=>{UI.dropZone.addEventListener(e,(()=>UI.dropZone.classList.add("dragover")))})),["dragleave","drop"].forEach((e=>{UI.dropZone.addEventListener(e,(()=>UI.dropZone.classList.remove("dragover")))})),UI.dropZone.addEventListener("drop",(e=>handleFiles(e.dataTransfer.files))),UI.fileInput.addEventListener("change",(e=>handleFiles(e.target.files)));async function handleFiles(e){const t=Array.from(e).filter((e=>e.type.startsWith("image/")));if(0===t.length)return;showLoader(!0,"Scanning...");for(const e of t)await processNewImage(e);renderPages(),toggleView(!0),showLoader(!1),UI.fileInput.value=""}function processNewImage(e){return new Promise((t=>{const o=new FileReader;o.onload=e=>{const r=new Image;r.onload=()=>{let e=r.width,o=r.height;const n=2e3;if(e>n||o>n){const t=Math.min(n/e,n/o);e=Math.floor(e*t),o=Math.floor(o*t)}UI.canvas.width=e,UI.canvas.height=o,ctx.drawImage(r,0,0,e,o);const a=UI.canvas.toDataURL("image/jpeg",.85);pages.push({id:crypto.randomUUID(),original:a,current:a,width:e,height:o,filter:"original",params:{threshold:15,contrast:0}}),t()},r.src=e.target.result},o.readAsDataURL(e)}))}function renderPages(){UI.pagesGrid.innerHTML="",UI.badgeCount.textContent=pages.length,0!==pages.length?(pages.forEach(((e,t)=>{const o="bw"===e.filter,r="magic"===e.filter,n=document.createElement("div");n.className="bg-gray-800 rounded-2xl overflow-hidden shadow-lg border border-gray-700 flex flex-col group relative",n.innerHTML=\`<div class="relative aspect-[3/4] bg-gray-900 overflow-hidden border-b border-gray-700"><img id="img-\${e.id}" src="\${e.current}" class="w-full h-full object-contain transition-opacity duration-300"><div class="absolute top-3 left-3 bg-black/60 backdrop-blur text-white text-[10px] font-bold px-2 py-1 rounded-md border border-white/10">\${t+1}</div><button onclick="window.open('\${e.current}')" class="absolute top-3 right-3 p-2 bg-black/60 backdrop-blur rounded-full text-white opacity-0 group-hover:opacity-100 transition hover:bg-blue-600"><i data-lucide="maximize-2" class="w-4 h-4"></i></button></div><div class="p-4 space-y-4"><div class="flex p-1 bg-gray-900 rounded-lg border border-gray-700">\${createFilterBtn(e.id,"original","Original",e.filter)}\${createFilterBtn(e.id,"magic","Magic",e.filter)}\${createFilterBtn(e.id,"bw","Scan B&W",e.filter)}</div><div class="h-10">\${o? \`<div class="space-y-1 animate-fadeIn"><div class="flex justify-between text-[10px] text-gray-400 font-medium uppercase tracking-wider"><span>Darker</span><span>Cleaner</span></div><input type="range" min="1" max="50" value="\${e.params.threshold}" oninput="debouncedUpdate('\${e.id}', 'bw', this.value)" class="w-full accent-blue-500"></div>\`:""}\${r? \`<div class="space-y-1 animate-fadeIn"><div class="flex justify-between text-[10px] text-gray-400 font-medium uppercase tracking-wider"><span>Soft</span><span>Vivid</span></div><input type="range" min="0" max="100" value="\${e.params.contrast}" oninput="debouncedUpdate('\${e.id}', 'magic', this.value)" class="w-full accent-purple-500"></div>\`:""}\${!o&&!r? '<p class="text-xs text-gray-600 text-center pt-2">No adjustments available</p>':""}</div><div class="pt-2 border-t border-gray-700 flex justify-end"><button onclick="removePage('\${e.id}')" class="text-red-400 hover:text-red-300 text-xs font-medium flex items-center gap-1.5 px-2 py-1 rounded hover:bg-red-900/20 transition"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i> Delete</button></div></div>\`,UI.pagesGrid.appendChild(n)}),lucide.createIcons()):toggleView(!1)}function createFilterBtn(e,t,o,r){const n=t===r,a=n?"bg-gray-700 text-white shadow":"text-gray-400 hover:text-gray-200 hover:bg-gray-800";return\`<button onclick="setFilter('\${e}', '\${t}')" class="flex-1 py-1.5 text-xs font-medium rounded-md transition-all \${a}">\${o}</button>\`}window.setFilter=(e,t)=>{const o=pages.find((t=>t.id===e));o&&o.filter!==t&&(o.filter=t,"bw"===t&&(o.params.threshold=15),"magic"===t&&(o.params.contrast=20),triggerUpdate(o))};let debounceTimer;function triggerUpdate(e){const t=document.getElementById(\`img-\${e.id}\`);t&&(t.style.opacity="0.4"),requestAnimationFrame((()=>{setTimeout((()=>{processImage(e),renderPages()}),10)}))}function processImage(e){const t=new Image;t.src=e.original,UI.canvas.width=e.width,UI.canvas.height=e.height,ctx.drawImage(t,0,0),"bw"===e.filter?applySmartScan(ctx,e.width,e.height,e.params.threshold):"magic"===e.filter&&applyMagicColor(ctx,e.width,e.height,e.params.contrast),e.current=UI.canvas.toDataURL("image/jpeg",.85)}function applySmartScan(e,t,o,r){const n=e.getImageData(0,0,t,o),a=n.data,i=new Uint8Array(t*o);for(let e=0;e<t*o;e++)i[e]=.299*a[4*e]+.587*a[4*e+1]+.114*a[4*e+2];const c=new Uint32Array(t*o);for(let e=0;e<o;e++){let r=0;for(let o=0;o<t;o++){const n=e*t+o;r+=i[n],0===e?c[n]=r:c[n]=r+c[(e-1)*t+o]}}const l=Math.floor(t/16),s=r/100;for(let e=0;e<o;e++)for(let r=0;r<t;r++){const o=Math.max(r-l,0),d=Math.min(r+l,t-1),u=Math.max(e-l,0),m=Math.min(e+l,o-1),g=(d-o)*(m-u),h=u>0&&o>0?c[(u-1)*t+(o-1)]:0,p=u>0?c[(u-1)*t+d]:0,f=o>0?c[m*t+(o-1)]:0,w=c[m*t+d],b=(w-p-f+h)/g,y=(e*t+r)*4,v=i[e*t+r]<b*(1-s)?0:255;a[y]=a[y+1]=a[y+2]=v}e.putImageData(n,0,0)}function applyMagicColor(e,t,o,r){const n=e.getImageData(0,0,t,o),a=n.data,i=1+r/100,c=128*(1-i);for(let e=0;e<a.length;e+=4){a[e]=a[e]*i+c,a[e+1]=a[e+1]*i+c,a[e+2]=a[e+2]*i+c;const t=.299*a[e]+.587*a[e+1]+.114*a[e+2];a[e]+=.5*(a[e]-t),a[e+1]+=.5*(a[e+1]-t),a[e+2]+=.5*(a[e+2]-t)}e.putImageData(n,0,0)}window.debouncedUpdate=(e,t,o)=>{clearTimeout(debounceTimer),debounceTimer=setTimeout((()=>{const r=pages.find((t=>t.id===e));"bw"===t&&(r.params.threshold=parseInt(o)),"magic"===t&&(r.params.contrast=parseInt(o)),triggerUpdate(r)}),150)};window.toggleView=toggleView;window.removePage=removePage;function toggleView(e){e?(UI.uploadView.classList.add("-translate-x-full"),UI.editorView.classList.remove("translate-x-full"),UI.resetBtn.classList.remove("hidden"),UI.badgeCount.classList.remove("hidden")):(UI.uploadView.classList.remove("-translate-x-full"),UI.editorView.classList.add("translate-x-full"),UI.resetBtn.classList.add("hidden"),UI.badgeCount.classList.add("hidden"))}function confirmReset(){confirm("Clear all scanned pages?")&&(pages=[],renderPages())}function showLoader(e,t=""){e?(UI.loader.classList.remove("hidden"),UI.loader.classList.remove("opacity-0"),UI.loaderText.innerText=t):(UI.loader.classList.add("opacity-0"),setTimeout((()=>UI.loader.classList.add("hidden")),300))}async function generatePDF(){if(0===pages.length)return;showLoader(!0,"Creating PDF...");await new Promise((e=>setTimeout(e,100)));try{const{jsPDF:e}=window.jspdf,t=new e,o=t.internal.pageSize.getWidth(),r=t.internal.pageSize.getHeight();pages.forEach(((e,n)=>{n>0&&t.addPage();const a=t.getImageProperties(e.current),i=a.width/a.height;let c=o,l=c/i;l>r&&(l=r,c=l*i);const s=(o-c)/2,d=(r-l)/2;t.addImage(e.current,"JPEG",s,d,c,l,void 0,"FAST")})),t.save("DocScanner_Pro.pdf")}catch(e){console.error(e),alert("Failed to generate PDF")}finally{showLoader(!1)}}</script></body></html>`,
        css: "",
        js: ""
    },
    {
